...
```{r}
districts %>% 
  ggplot(aes(y=region, weight=enrollment)) +
  geom_bar()
```
...
> It's even worse! Only Bradford SSD has eng 3 higher than eng 2
...
> the R squared is 0.079, very close to 0, so the linear fit does not match up to the data well
